...
export class ArtillerySkill extends AIUnitSkill {
    ...
    public aim(origin: vec2, tiles: vec2[]): vec2 | null {
        return this.active ? this.target : super.aim(origin, tiles)
    }
    ...
    private *launch(target: vec2): Generator<ActionSignal> {
        ...
        this.burn.transform = this.context.get(TransformSystem).create(targetPosition)
        ...
        this.pillar.transform = this.context.get(TransformSystem).create(targetPosition)
        ...
        .create(vec3.add(targetPosition, [0,2,0], vec3()))
        ...
        const controlB = vec3.set(targetPosition[0], controlA[1], targetPosition[2], vec3())
        vec3.centroid(controlB, controlA, controlB)
        const curve = FollowPath.curve(
            cubicBezier3D.bind(null, origin, controlA, controlB, targetPosition),

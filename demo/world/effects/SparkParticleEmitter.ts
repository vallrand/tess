...
import { GL, ShaderProgram, UniformBlock } from '../../engine/webgl'
...
layout(location=0) in vec4 aTranslate;
...
void main(){
    ...
    if(gl_VertexID > uLastID){
#if defined(SPHERE)
        vec3 random = vec2(hash(seed), hash(seed*16807U), hash(seed*48271U));
        ...
        vTransform = vec4(uOrigin + radius * normal,0);
#endif
...
export class SparkParticleEmitter {
    ...
    constructor(context: Application){
        ...
        const ps = new ParticleSystem(context, SparkParticleEmitter.stripeMesh(context.gl, 10), 
        ...
        //context.get(ParticleEffectPass).particleSystems.push(ps)
    }
    emit(){

...
export const vec2 = (x: number = 0, y: number = x) => vec2.set(x, y, new Float32Array(2) as any)

vec2.set = (x: number, y: number, out: vec2): vec2 => {
    out[0] = x
    out[1] = y
    return out
}
...
vec2.rotate = (vec: vec2, angle: number, out: vec2): vec2 => {
